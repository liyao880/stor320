...
By default, the functon uses 'loess' method. If you would like to use another method, such as `lm`.

```{r}
p2+geom_smooth(aes(x=year,y=demVote,color=as.factor(south)), method = 'lm')
```

Not just linear regression, with formula, you can also include higher order terms.
```{r}
